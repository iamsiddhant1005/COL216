    ...
    while(j<k+4){
       ...
       if(cycle>=4) {
            if(var_delay[p-4]!=0){
                ...
                if(ins_number[p-4]==k-1){
                    
                    j=j-1;
                    instruction[p-3]=instruction[p-4];
                    ins_number[p-3]=ins_number[p-4];
                    mux_MemRead[p-3]=mux_MemRead[p-4];
                    mux_MemWrite[p-3]=mux_MemWrite[p-4];
                    value[p-3]=value[p-4];
                    rt[p-3]=rt[p-4];
                    dest[p-3]=dest[p-4];
                }
                else if(ins_number[p-4]==k-2){
                    // cout<<"**"<<endl;
                    j=j-1;
                    instruction[p-2]=instruction[p-3];
                    instruction[p-3]=instruction[p-4];
                    ins_number[p-2]=ins_number[p-3];
                    ins_number[p-3]=ins_number[p-4];
                    opcode[p-2]=read_instructions(instruction,rd,rs,rt,sa,offset,imm,mux_RegDst,mux_AluSrc,p-2);
                    mux_MemRead[p-3]=mux_MemRead[p-4];
                    mux_MemWrite[p-3]=mux_MemWrite[p-4];
                    value[p-3]=value[p-4];
                    rt[p-3]=rt[p-4];
                ...
                else if(ins_number[p-4]==k-3){
                    j=j-1;
                    instruction[p-1]=instruction[p-2];
                    instruction[p-2]=instruction[p-3];
                    instruction[p-3]=instruction[p-4];
                    ins_number[p-1]=ins_number[p-2];
                    ins_number[p-2]=ins_number[p-3];
                    ins_number[p-3]=ins_number[p-4];
                    opcode[p-2]=read_instructions(instruction,rd,rs,rt,sa,offset,imm,mux_RegDst,mux_AluSrc,p-2);
                    mux_MemRead[p-3]=mux_MemRead[p-4];
           ...
           //  mux_RegWrite=0;
            if(bubble[p-3]==1){
               ...
               // cout<<value[p-3]<<endl;
                ...
                if(mux_MemRead[p-3]==1 || mux_MemWrite[p-3]==1){
                    if(var_delay[p-3]==0){
                       // float miss=(float) (rand()%100)/100;
                        if(var_delay[p-4]!=1){
                            float miss=(float) (rand()%100)/100;
                            cout<<miss<<endl;
                            if(miss>=0.05){
                                for(int del=0;del<max_delay-1;del++){
                                        var_delay[p-3+del]=1;
                                    }
                            }
                       } 
      ...
      // cout<<p<<" "<<bubble[p]<<endl;
       ...
       //cout<<"j= "<<j<<endl;
       //cout<<reg[11]<<endl;
       //if(p>=4)cout<<var_delay[p-4]<<endl;
